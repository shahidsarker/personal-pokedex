...
let loadPokemon = (trainer, pokemonIndex) => {
  ...
  hpStatBar.setAttribute(
    "style",
    `width:${(trainer.all()[pokemonIndex].hp / 255) * 100}%;`
  );
  ...
  attackStatBar.setAttribute(
    "style",
    `width:${(trainer.all()[pokemonIndex].attack / 190) * 100}%;`
  );
  ...
  defenseStatBar.setAttribute(
    "style",
    `width:${(trainer.all()[pokemonIndex].defense / 230) * 100}%;`
  );
  ...
  pokeabilities.innerText = `${trainer
    .all()
    [pokemonIndex].ability[0].split("-")
    .join(" ")
    .toProperCase()}, ${trainer
    .all()
    [pokemonIndex].ability[1].split("-")
    .join(" ")}`;

  let pokeType = document.getElementById("pokemon-type");
  pokeType.innerHTML = `<span class="badge badge-primary ${
    trainer.all()[pokemonIndex].types[0]
  }-type">${trainer.all()[pokemonIndex].types[0]}</span>`;
};
